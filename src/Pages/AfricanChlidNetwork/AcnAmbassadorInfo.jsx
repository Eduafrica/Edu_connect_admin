...
import { deleteAmbassdor, toggleAmbassdorActiveStatus } from "../../Helpers/acn/api";

function AcnAmbassadorInfo() {
    const navigate = useNavigate()
    const loc = useLocation()
    const pathName = loc.pathname.split('/')[4]

    const { data: teamMemberData, isFetching } = useFetchAmbassadors(pathName)
    ...
    const handleDelete = async () => {
        ...
        if(confirm){
            try {
                setLoading(true)
                const res = await deleteAmbassdor({ id: pathName })
                if(res.success){
                    toast.success(res.data)
                    navigate('/acn/ambassdor')
                } else {
    ...
    const handleApprove = async () => {
        ...
        if(confirm){
            try {
                setLoading(true)
                const res = await toggleAmbassdorActiveStatus({ id: pathName })
                if(res.success){
                    toast.success(res.data)
...
export default AcnAmbassadorInfo;
