...
function TopDonations({ text, data, loading }) {
  return (
    <div className="w-[618px] border-[1px] border-[#F1F1F1] rounded-[10px] shadow-sm">
        ...
        <div className="w-full flex items-center gap-[50px]">
          <div className="flex items-center min-w-[140px] w-full pt-[20px] pl-[25px]">
            <h2 className="text-lg font-semibold text-[#121212] w-full">{ text ? text : `Top 5 Donations` }</h2>
          </div>
        </div>
              ...
              data?.map((item) => {
                ...
                return (
                  <tr key={item?._id} className="border-t border-gray-200">
                    ...
                    {/* donation amount */}
                    <td className="px-6 py-4">
                      <div className="text-[18px] text-start font-medium text-[#06152B] flex-col">
                        {item?.donationType}
                      </div>
                    </td>
                    {/* More */}
                    <td className="px-6 py-4">
                      <Link to={`/acn/donation/info/${item?.donationId}`} className="flex items-center gap-2">
